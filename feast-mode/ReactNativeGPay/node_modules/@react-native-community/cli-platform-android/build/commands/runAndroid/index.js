...
function runAndroid(argv, config, args) {
  ...
  if (args.jetifier) {
    _cliTools().logger.info(`Running ${_chalk().default.bold('jetifier')} to migrate libraries to AndroidX. ${_chalk().default.dim('You can disable it using "--no-jetifier" flag.')}`); // Jetifier is a side-effectful module without a default export. Requiring
    // it ad-hoc.
